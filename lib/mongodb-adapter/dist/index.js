...
function MongoDBAdapter(client, options = {}) {
  ...
  const db = (async () => {
    ...
    return {
      ...
      V: _db.collection(
        c === null || c === void 0 ? void 0 : c.VerificationTokens
      ),
    };
  })();
  return {
    async createUser(data) {
      ...
      await (
        ...
        apiKey: crypto
          .createHash("sha512")
          .update(JSON.stringify(user.email) + process.env.USER_AUTH_KEY)
          .digest("hex"),
      });
      return from(user);
    },
    ...
    async getUserByAccount(provider_providerAccountId) {
      ...
      const user = await (
        await db
      ).U.findOne({ _id: new mongodb_1.ObjectId(account.userId) });
      if (!user) return null;
      return from(user);
    },
    ...
    async getSessionAndUser(sessionToken) {
      ...
      const user = await (
        await db
      ).U.findOne({ _id: new mongodb_1.ObjectId(session.userId) });
      if (!user) return null;
      return {
        user: from(user),
